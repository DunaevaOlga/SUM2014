...
static VOID ClockUnitRender( od6UNIT_CLOCK *Unit, od6ANIM *Ani )
{
  ...
  FLOAT PI = 3.14159265358979323846;
    ...
    DrawArrow(Ani->hDC, Ani->W / 2, Ani->H / 2, Ani->W * Ani->H / 9000, 20, (-(st.wHour % 12 + st.wMinute / 60.0) / 12.0) * 2 * PI);
    DrawArrow(Ani->hDC, Ani->W / 2, Ani->H / 2, Ani->W * Ani->H / 7500, 15, (-(st.wMinute + st.wSecond / 60.0) / 60.0) * 2 * PI);
    DrawArrow(Ani->hDC, Ani->W / 2, Ani->H / 2, Ani->W * Ani->H / 6000, 10, (-(st.wSecond + st.wMilliseconds / 1000.0) / 60.0) * 2 * PI);
    DrawArrow(Ani->hDC, Ani->W / 2, Ani->H / 2, Ani->W * Ani->H / 5000, 3, (-st.wMilliseconds / 1000.0) * 2 * PI);
 /* DBL
...
static VOID InfoUnitRender( od6UNIT_INFO *Unit, od6ANIM *Ani )
{
  ...
  x = (Ani->W - Unit->And.W) / 2;
  y = (Ani->H - Unit->And.H) / 2;
