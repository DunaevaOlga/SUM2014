...
*/
static VOID LoadMaterials( od6GEOM *G, CHAR *FileName )
{
  FILE *F;
  od6MATERIAL DefMat, Mat;
...
BOOL OD6_GeomLoad( od6GEOM *G, CHAR *FileName )
{
  ...
  size = 
    ...
    sizeof(od6UV) * vtn +                     /*   текстурные координаты vtn */
  ...
  OD6_DefaultColor = ColorSet(1, 1, 1);
  for (p = 0; p < pn; p++)
  {
    INT minv, maxv, j;
    od6PRIM prim;
    ...
    OD6_PrimCreate(&prim, OD6_PRIM_TRIMESH, vn, fn * 3);
    ...
    if (is_need_normal)
      OD6_PrimAutoNormals(&prim);
    prim.Mtl = PrimInfo[p].Mtl;
    OD6_GeomAddPrim(G, &prim);
  }
  /* освобождаем память из-под прочитанных данных */
  free(ReadV);
  return TRUE;
} /* End of 'OD6_GeomLoad' function */
